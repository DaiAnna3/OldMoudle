...
# 子图1：spower_PIDE 与 α_PIDE 的散点图及线性拟合曲线
ax1 = axes[0]
ax1.scatter(spower_PIDE, α_PIDE, s=80, c='skyblue', label='PIDE数据库α', marker='o', edgecolors='k', linewidths=1)
ax1.scatter(spower_PIDE, α_predict, s=80, c='orange', label='模型预测α', marker='x', edgecolors='k', linewidths=1)

# 进行线性拟合
coeffs1_pred = np.polyfit(spower_PIDE, α_predict, 1)
poly1_pred = np.poly1d(coeffs1_pred)
ax1.plot(spower_PIDE, poly1_pred(spower_PIDE), linewidth=2, linestyle='--', color='red', label='α 预测 线性拟合')

coeffs1_act = np.polyfit(spower_PIDE, α_PIDE, 1)
poly1_act = np.poly1d(coeffs1_act)
ax1.plot(spower_PIDE, poly1_act(spower_PIDE), linewidth=2, linestyle=':', color='blue', label='α 实际 线性拟合')

# 标注斜率
slope_text1_pred = f"预测斜率: {coeffs1_pred[0]:.3f}"
slope_text1_act = f"实际斜率: {coeffs1_act[0]:.3f}"
ax1.text(0.05, 0.9, slope_text1_pred, transform=ax1.transAxes, fontsize=12, color='red')
ax1.text(0.05, 0.8, slope_text1_act, transform=ax1.transAxes, fontsize=12, color='blue')

ax1.set_xlabel('spower_PIDE')
ax1.set_ylabel('α value')
ax1.legend()


# 子图2：spower_PIDE 与 β_PIDE 的散点图及线性拟合曲线
ax2 = axes[1]
ax2.scatter(spower_PIDE, β_PIDE, s=80, c='lightgreen', label='PIDE数据库β', marker='s', edgecolors='k', linewidths=1)
ax2.scatter(spower_PIDE, β_predict, s=80, c='purple', label='模型预测β', marker='x', edgecolors='k', linewidths=1)

# 进行线性拟合
coeffs2_pred = np.polyfit(spower_PIDE, β_predict, 1)
poly2_pred = np.poly1d(coeffs2_pred)
ax2.plot(spower_PIDE, poly2_pred(spower_PIDE), linewidth=2, linestyle='--', color='red', label='β 预测 线性拟合')

coeffs2_act = np.polyfit(spower_PIDE, β_PIDE, 1)
poly2_act = np.poly1d(coeffs2_act)
ax2.plot(spower_PIDE, poly2_act(spower_PIDE), linewidth=2, linestyle=':', color='blue', label='β 实际 线性拟合')

# 标注斜率
slope_text2_pred = f"预测斜率: {coeffs2_pred[0]:.3f}"
slope_text2_act = f"实际斜率: {coeffs2_act[0]:.3f}"
ax2.text(0.05, 0.9, slope_text2_pred, transform=ax2.transAxes, fontsize=12, color='red')
ax2.text(0.05, 0.8, slope_text2_act, transform=ax2.transAxes, fontsize=12, color='blue')


ax2.set_xlabel('spower_PIDE')
ax2.set_ylabel('β value')
ax2.legend()
